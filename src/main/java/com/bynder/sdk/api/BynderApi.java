...
public interface BynderApi {
  ...
  /**
   * Gets a temporary access token pair once the user already authorised the request token pair. If
   * successful the request token pair is immediately expired.
   *
  ...
  /**
   * Initialises a file upload with Bynder and returns authorisation information to allow uploading to
   * the Amazon S3 bucket-endpoint.
   *
   * @param params {@link FieldMap} with parameters.
  ...
  /**
   * Gets the URL of the Amazon S3 bucket-endpoint in the region closest to the server. This URL is
   * used to upload parts to Amazon.
   *
