...
public class AppSample {
    ...
    public void listItems() {
        bynderClient.getDerivatives().map(RXUtils::getResponseBody).subscribe(
                derivatives -> LOG.info("Derivatives: " + derivatives.stream().map(Derivative::getPrefix).collect(Collectors.toList())),
                this::logError
        ).dispose();

        assetService.getBrands().map(RXUtils::getResponseBody).subscribe(
        ...
        assetService.getMediaList(new MediaQuery()
                ...
                .setLimit(10)
                .setPage(1)
        ).map(RXUtils::getResponseBody).subscribe(
    ...
    public void uploadFile(final String uploadPath) {
        assetService.getBrands().singleOrError().map(RXUtils::getResponseBody).flatMap(brands ->
            ...
            return assetService.getMediaInfo(new MediaInfoQuery(saveMediaResponse.getMediaId()).setVersions(true)).map(RXUtils::getResponseBody).retryWhen(f ->
