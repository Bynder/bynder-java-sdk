...
public class OAuthServiceImpl implements OAuthService {
    ...
    public Single<Token> getAccessToken(final String code) {
        return oauthClient.getAccessToken(queryDecoder.decode(
                new AccessTokenQuery(oAuthSettings, code)
        )).map(RXUtils::getResponseBody).map(this::setToken);
    }
    ...
    public Single<Token> refreshAccessToken() {
        ...
        if (refreshToken != null) {
            ...
            )).map(RXUtils::getResponseBody).map(newToken -> setToken(newToken).setRefreshToken(refreshToken));
        } else {
